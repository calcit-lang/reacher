...
{}
  ...
  :entries $ {} $ :default
    {} (:description |)
      ...
      :mode :js
      :reload-fn 'reacher.app.main/reload!
      :feature-policy $ {}
  ...
  :files $ {}
    'reacher.app.comp.container $ %{} 'FileEntry
      :defs $ {}
        'comp-container $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn comp-container (props ? children)
            let
        ...
        'comp-task $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn comp-task (? props children)
            use-effect!
              [] $ browser/random
              fn () $ println |effect
      ...
      :defs $ {}
        ...
        'mount-target $ %{} 'CodeEntry (:doc |)
          :code $ quote $ def mount-target
            createRoot $ js/document.querySelector |.app
          :examples $ []
          :schema $ :: 'Dynamic
        ...
        'render-app! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn render-app! ()
            .!render mount-target $ wrap-comp dispatch-provider
    ...
    'reacher.app.updater $ %{} 'FileEntry
      :defs $ {} $ 'updater
        %{} 'CodeEntry (:doc |)
          :code $ quote $ defn updater (store op data op-id op-time)
            case-default op
              do (println "|unknown op:" op) store
              :add-task $ update store :tasks $ fn (tasks)
                conj tasks $ {} (:id op-id) (:time op-time) (:done? false) (:text data)
              :rm-task $ update store :tasks $ fn (tasks)
                -> tasks $ filter $ fn (task)
                  not= data $
                    get task :id
                    , .unwrap-or 0
              :toggle-task $ update store :tasks $ fn (tasks)
                -> tasks $ map $ fn (task)
                  if
                    = data $
                      get task :id
    ...
    'reacher.core $ %{} 'FileEntry
      :defs $ {}
        ...
        'context-of-dispatch $ %{} 'CodeEntry (:doc |)
          :code $ quote $ def context-of-dispatch
            React/createContext $ fn (op data)
              js/console.warn "|missing dispatch function" op
          :examples $ []
          :schema $ :: 'Dynamic
        ...
        'use-dispatch $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn use-dispatch ()
            React/useContext context-of-dispatch
          :examples $ []
          :schema $ :: 'Dynamic
        ...
        'wrap-comp $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn wrap-comp (f props & children)
        ...
        'read-store-prop $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn read-store-prop (props)
            let
              ...
              if (map? value)
                unsafe-coerce value $ :: 'Map 'Tag $ :: 'List (:: 'Map 'Tag 'Dynamic)
                raise |JS_FFI_contract_violation:_component.props.store_expected_Map
          :examples $ []
          :schema $ :: 'Fn $ {}
            :args $ [] $ :: 'JsNullish 'JsObject
            :features $ #{} :js-ffi
            :return $ :: 'Map 'Tag $ :: 'List (:: 'Map 'Tag 'Dynamic)
    ...
    'reacher.util.str $ %{} 'FileEntry
      :defs $ {}
        ...
        'dashed-letter-pattern $ %{} 'CodeEntry (:doc |)
          :code $ quote $ def dashed-letter-pattern (new js/RegExp |-[a-z] |g)
          :examples $ []
          :schema $ :: 'Dynamic
        ...
        'get-style-value $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn get-style-value (x prop)
            cond
              ...
              (number? x)
                if
                  .!test pattern-non-dimension-props prop
                  str x
                  str x |px
              true $ str x
          :examples $ []
